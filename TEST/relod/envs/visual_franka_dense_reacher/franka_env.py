...
class FrankaPanda_Visual_Reacher_V0(gym.Env):
    ...
    """
    def __init__(self, dt=0.04, image_history_size=3, image_width=160, image_height=90, camera_index=0, seed=9):
        np.random.seed(seed)
        self.DT= dt
        self.dt = dt
        ...
        # self.camera = camera()
        ####
        self.camera = FastCamera(self._image_width, self._image_height, camera_index)
    ...
    def reset(self):
        ...
        reset_pose['panda_joint4'] = np.random.uniform(-2.05, -1.9)
...
class FastCamera():
    def __init__(self, image_width, image_height, camera_index):
        ...
        self.cap.set(cv2.CAP_PROP_FOURCC, 1196444237.0)
        ...
        #     _, image = self.cap.read() 
            # path = "/home/franka/project/ReLoDFranka/rl655/images/" + str(i) + '.png'
            ...
            # mask = cv2.dilate(mask, kernel, iterations=5)
            # mask = cv2.erode(mask, kernel, iterations=5)
            # path = "/home/franka/project/ReLoDFranka/rl655/images/b" + str(i) + '.png'

            # target_size = np.sum(mask/255.) / mask.size
            # print("Mask", i, ": ", target_size)
